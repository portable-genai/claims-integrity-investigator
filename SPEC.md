# SPEC: Claims Integrity Investigator (`claims-integrity-investigator`)
...
- **Maker-checker (P-06) and routing (R8)**: a HIGH/CRITICAL result sets
  ...
  with routing on and no console configured; `CLAIMSINTEG_REVIEW_ROUTING=off` is the stated way to
...
- **Profile**: resolved ONCE, at import, into a `ProfileChoice` and never a bare string. Three
  states of `CLAIMSINTEG_PROFILE`: UNSET is NO CHOICE (the SDK-free adapters

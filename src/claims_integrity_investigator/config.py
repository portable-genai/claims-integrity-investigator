...
One env var (``CLAIMSINTEG_PROFILE``) selects the adapter family for every
...
from .domain.policy import AssessmentPolicy
from .envread import boolean_setting, setting_or_default
from .ports.audit import AuditSinkPort
from .ports.claim_file import ClaimFilePort
from .ports.claims_history import ClaimsHistoryPort
from .ports.extraction import ExtractionPort
from .ports.fraud_linkage import FraudLinkagePort
...
from .ports.policy_corpus import PolicyCorpusPort
from .ports.review_router import ReviewRouterPort

_PROFILE_ENV = "CLAIMSINTEG_PROFILE"
_SETTINGS_ENV = "CLAIMSINTEG_SETTINGS"
...
class ProfileChoice:
    ...
    The variable is ``CLAIMSINTEG_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("CLAIMSINTEG_PROFILE", "local")``: that fallback reads an
...
_PKG = "claims_integrity_investigator"
...
DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    ...
    "claim_file": {
        "local": f"{_PKG}.adapters.local.claim_file:LocalClaimFileAdapter",
        "gcp": f"{_PKG}.adapters.gcp.claim_file:CloudClaimFileAdapter",
        "onprem": f"{_PKG}.adapters.onprem.claim_file:OnPremClaimFileAdapter",
    },
    "claims_history": {
        "local": f"{_PKG}.adapters.local.claims_history:LocalClaimsHistoryAdapter",
        "gcp": f"{_PKG}.adapters.gcp.claims_history:CloudClaimsHistoryAdapter",
        "onprem": f"{_PKG}.adapters.onprem.claims_history:OnPremClaimsHistoryAdapter",
    },
    "extraction": {
    ...
    "fraud_linkage": {
        "local": f"{_PKG}.adapters.local.fraud_linkage:LocalFraudLinkageAdapter",
        "gcp": f"{_PKG}.adapters.gcp.fraud_linkage:CloudFraudLinkageAdapter",
        "onprem": f"{_PKG}.adapters.onprem.fraud_linkage:OnPremFraudLinkageAdapter",
    },
    "generation": {
    ...
    "policy_corpus": {
        "local": f"{_PKG}.adapters.local.policy_corpus:LocalPolicyCorpusAdapter",
        "gcp": f"{_PKG}.adapters.gcp.policy_corpus:Hrz2PolicyCorpusAdapter",
        "onprem": f"{_PKG}.adapters.onprem.policy_corpus:OnPremPolicyCorpusAdapter",
    },
    "review_router": {
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``CLAIMSINTEG_SETTINGS``) that does not
...
def _policy_from(block: Any) -> AssessmentPolicy:
    """Build the frozen :class:`AssessmentPolicy` from the settings ``policy:`` block.

    An absent block takes the shipped defaults. A present block overrides only the keys it
    names, so an adopter tunes one threshold without restating the whole policy. Unknown keys
    raise: a mis-typed threshold that silently did nothing is how a control drifts unnoticed.
    """
    if block is None:
        return AssessmentPolicy()
    if not isinstance(block, Mapping):
        raise ValueError("settings 'policy' must be a mapping of threshold -> value")
    default = AssessmentPolicy()
    fields = {f.name for f in dataclass_fields(default)}
    unknown = set(block) - fields
    if unknown:
        raise ValueError(f"settings 'policy' has unknown keys: {sorted(unknown)}")
    scalars = {
        key: value for key, value in block.items() if key not in {"uplifts", "staged_loss_markers"}
    }
    uplifts = block.get("uplifts")
    markers = block.get("staged_loss_markers")
    return replace(
        default,
        **scalars,
        uplifts={str(k): float(v) for k, v in dict(uplifts).items()}
        if uplifts
        else default.uplifts,
        staged_loss_markers=(
            tuple(str(m) for m in markers) if markers else default.staged_loss_markers
        ),
    )
...
REVIEW_ROUTING_ENV = "CLAIMSINTEG_REVIEW_ROUTING"
...
class Container:
    ...
    @cached_property
    def claim_file(self) -> ClaimFilePort:
        adapter = self._bind("claim_file")
        assert isinstance(adapter, ClaimFilePort)
        return adapter

    @cached_property
    def claims_history(self) -> ClaimsHistoryPort:
        adapter = self._bind("claims_history")
        assert isinstance(adapter, ClaimsHistoryPort)
        return adapter
    ...
    @cached_property
    def fraud_linkage(self) -> FraudLinkagePort:
        adapter = self._bind("fraud_linkage")
        assert isinstance(adapter, FraudLinkagePort)
        return adapter
    ...
    @cached_property
    def policy_corpus(self) -> PolicyCorpusPort:
        adapter = self._bind("policy_corpus")
        assert isinstance(adapter, PolicyCorpusPort)
        return adapter

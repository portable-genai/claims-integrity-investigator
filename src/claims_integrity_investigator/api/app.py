...
``CLAIMSINTEG_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``CLAIMSINTEG_S2S_TOKEN`` is wrong in the most dangerous direction:
...
from .schemas import AssessRequest, AssessResponse, HealthResponse, SiuQueueItem
...
_INSECURE_DEMO_ENV = "CLAIMSINTEG_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "CLAIMSINTEG_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "CLAIMSINTEG_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "CLAIMSINTEG_S2S_AUDIENCE"
_API_HOST_ENV = "CLAIMSINTEG_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``CLAIMSINTEG_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``CLAIMSINTEG_PROFILE`` must never be allowed to
    ...
    ``CLAIMSINTEG_ALLOW_INSECURE_DEMO=1``.
    """
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "CLAIMSINTEG_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "claims-integrity-investigator"
...
_APP_TITLE = "Claims Integrity Investigator"
...
#
# Note what is NOT in this expression: CLAIMSINTEG_S2S_TOKEN. A service
...
def assess(
    ...
    try:
        result = service.assess(request.claim_id, actor=principal.actor, tenant=principal.tenant)
    except KeyError as exc:
        # 404, not 403: a claim owned by another tenant must be indistinguishable from one that
...
@app.get("/v1/siu-queue", response_model=list[SiuQueueItem], tags=["artifacts"])
def siu_queue(
    principal: Annotated[Principal, Depends(get_principal)],
) -> list[SiuQueueItem]:
    """List the SIU-referral / investigate items currently routed to human review.

    Reads the review router's pending queue (the local outbox offline) and reports the items a
    reviewer still owns. It manufactures nothing: an empty queue means nothing has been routed in
    this process, which is the honest offline answer.
    """
    router = _container().review_router
    pending = getattr(router, "outbox", None)
    if pending is None:
        return []
    items: list[SiuQueueItem] = []
    for entry in pending.pending():
        review = entry.review
        # Scoped to the principal's tenant. The queue is a query, and an unscoped query is the
        # read-by-id defect with no id to guess: it showed a reviewer at one insurer every claim
        # another insurer had routed, subject line included.
        if review.tenant != principal.tenant:
            continue
        items.append(
            SiuQueueItem(

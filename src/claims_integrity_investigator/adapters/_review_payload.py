...
def _kit_citations(result: ClaimAssessment) -> tuple[KitCitation, ...]:
    seen: set[str] = set()
...
def result_to_review(result: ClaimAssessment, *, maker: str, tenant: str = "") -> Review:
    """Build the review a producer submits to Hrz7 when an assessment escalates."""
    return Review(
        action="claims_integrity_investigator:assess_claim",
        ...
        sod_group="claims_integrity_investigator-maker-checker",
        case_ref=result.claim_id,
        # Producer-owned, tenant-scoped key so a retried delivery is idempotent at the console.
        source_key=f"Ins1:{result.claim_id}:{result.recommendation.value}",

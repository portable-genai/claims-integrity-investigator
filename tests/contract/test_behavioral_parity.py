...
from claims_integrity_investigator.adapters._review_payload import (
    result_to_review,
)
...
def test_the_payload_that_reaches_the_wire_is_redacted_whichever_family_built_it() -> None:
    ...
    result = sample_cases.planted_pii_assessment()
    payload = repr(
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    ...
    result = build_assessment_service(local).assess(
    ...
    with pytest.raises(NotImplementedError):
        build_assessment_service(onprem).assess(
            sample_cases.SIU_CLAIM, actor=sample_cases.ACTOR, tenant=FIXTURE_TENANT
        )

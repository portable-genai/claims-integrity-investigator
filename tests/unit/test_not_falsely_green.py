...
The green and red inputs are REAL persisted audit rows from the real pipeline: the same
assessment, once with the redaction boundary in place and once with the raw identifier put back
into the row the way an unmasked citation puts it there.
"""
...
from claims_integrity_investigator.domain.assessment_service import build_assessment_service

from tests.fixtures import sample_cases


def _persisted_rows() -> list[dict[str, Any]]:
    """Run the REAL pipeline over a fixture claim; return the WORM rows it wrote."""
    container = build_container(Settings(profile="local", audit_path=":memory:"))
    build_assessment_service(container).assess(
        "CLM-1004", actor=sample_cases.ACTOR, tenant=FIXTURE_TENANT
    )
...
def _unredacted(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    same WORM record, which is precisely what the G-series linkage note did.
    """
    mutated = copy.deepcopy(rows)
    for row in mutated:
        citations = row.get("citations") or []
        if citations:
            citations[0]["snippet"] = f"claimant NRIC {sample_cases.PLANTED_NRIC} on file"
    return mutated

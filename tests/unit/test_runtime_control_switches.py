...
control this service has: ``CLAIMSINTEG_REVIEW_ROUTING`` is read in three states; off binds a
...
from claims_integrity_investigator.adapters.controls import (
    DisabledReviewRouter,
    RecordingReviewRouter,
    ReviewRouting,
)
from claims_integrity_investigator.agent import tools
from claims_integrity_investigator.api import app as api_module
from claims_integrity_investigator.api.app import app
from claims_integrity_investigator.cli import main as cli
...
_PATH = "/v1/assess"
_ESCALATING: dict[str, Any] = {"claim_id": "CLM-1004"}
_HEADERS = {"X-Dev-Persona": "auditor"}
_LOCAL_ROUTER = "claims_integrity_investigator.adapters.local.review_router.LocalReviewRouter.route"
...
def test_a_failed_hand_off_is_reported_and_logged_never_raised(
    ...
    failed = RecordingReviewRouter(_Refusing())
    with caplog.at_level(logging.WARNING, logger="claims_integrity_investigator.adapters.controls"):
...
def test_the_agent_tool_reports_the_hand_off() -> None:
    payload = tools.assess_claim("CLM-1004", tenant="demo-bank")
...
def test_the_cli_reports_the_hand_off(
    ...
    monkeypatch.setenv(REVIEW_ROUTING_ENV, "off")
    assert cli.main(["assess", "CLM-1004", "--tenant", "demo-bank"]) == 0

...
from claims_integrity_investigator.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from claims_integrity_investigator.config import Settings
from claims_integrity_investigator.ports.identity import (
    VERIFIED,
...
_API_MODULE = "claims_integrity_investigator.api.app"
...
def _local_rebound(port: str, local_cls: str, onprem_cls: str) -> list[str]:
    ...
    return [
        f"  {port}:",
        *[
            f"    {p}: {_PKG}.adapters.local.{port.split('.')[0]}:{local_cls}"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.{port.split('.')[0]}:{onprem_cls}",
    ]
...
_REBOUND_SETTINGS = "\n".join(
    [
        ...
        "adapters:",
        *_local_rebound("audit", "LocalAuditAdapter", "OnPremAuditAdapter"),
        *_local_rebound("claim_file", "LocalClaimFileAdapter", "OnPremClaimFileAdapter"),
        *_local_rebound(
            "claims_history", "LocalClaimsHistoryAdapter", "OnPremClaimsHistoryAdapter"
        ),
        ...
        *_local_rebound("policy_corpus", "LocalPolicyCorpusAdapter", "OnPremPolicyCorpusAdapter"),
        *_local_rebound("review_router", "LocalReviewRouter", "OnPremReviewRouter"),
        # Every port must bind every profile or `_bindings_from` refuses the whole file.
        # These take no part in the identity posture under test.
        *_local_rebound("tracer", "LocalNoopTracerAdapter", "OnPremTracerAdapter"),
        *_local_rebound("evaluation", "LocalOfflineEvalAdapter", "OnPremEvalAdapter"),
    ]
)
...
def test_a_verified_assertion_is_served_so_the_refusals_above_mean_something(
    ...
    """
    # The `hd` claim IS the tenant this adapter derives, and the claim-file port matches it
    # against each file's data tag, so the positive control has to assert a principal who
    # actually owns the fixture rows. A control that 404s on object-level authorization would
    # still "pass" the identity assertions while proving nothing about them.
    install_fake_google(monkeypatch, {**GOOD_CLAIMS, "hd": FIXTURE_TENANT})
    module = _managed_app(monkeypatch, tmp_path, AUDIENCE)
    with TestClient(module.app, client=(LAN_PEER, 50000)) as client:
        resp = client.post(
            "/v1/assess",

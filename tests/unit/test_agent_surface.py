...
from claims_integrity_investigator.agent import (
    SKILLS,
    TOOL_FUNCTIONS,
    agent_card_document,
    assess_claim,
...
def test_the_card_document_is_json_safe_and_names_the_region() -> None:
    ...
    assert document["name"] == "claims-integrity-investigator"
...
def test_the_agent_path_routes_an_escalation_rather_than_only_flagging_it() -> None:
    ...
    assert result["recommendation"] == "siu_refer"


def test_the_siu_queue_tool_reads_the_queue_rather_than_manufacturing_one() -> None:
    settings = local_settings()
    # A fresh container has an empty queue; nothing was routed in this settings' process.
    empty = list_siu_queue(settings)
...
def test_the_tool_output_is_masked_before_it_can_reach_a_model() -> None:
    ...
    The claim file carries a synthetic NRIC in its FNOL free text; the tool result must not
    carry it onward into a model's context. The redaction runs before the audit write and again
    on the tool payload, so the identifier never reaches here.
    """
    result = assess_claim(

...
def test_the_demo_data_is_obviously_fictional() -> None:
    ...
    claims = (demo.ACCEPT_CLAIM, demo.SIU_CLAIM, demo.REDACTION_CLAIM)
    blob = demo.ACTOR
    for claim_id in claims:
        claim = CLAIM_FILES[claim_id]
        assert "FICTIONAL" in claim.subject, claim.subject + " does not announce itself as fake"
        blob += claim.subject + " " + " ".join(doc.text for doc in claim.documents)

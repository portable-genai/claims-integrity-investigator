...
from claims_integrity_investigator.adapters.local.audit import (
    LocalAuditAdapter,
)
...
_CLAIMS = (sample_cases.SIU_CLAIM, sample_cases.ACCEPT_CLAIM, sample_cases.INVESTIGATE_CLAIM)

_ANCHOR_ENV = "CLAIMSINTEG_AUDIT_ANCHOR"
...
def _record_three(settings: Settings) -> LocalAuditAdapter:
    ...
    for claim_id in _CLAIMS:
        service.assess(claim_id, actor=sample_cases.ACTOR, tenant=FIXTURE_TENANT)
    return audit
...
def test_an_append_after_truncation_cannot_relaunder_the_anchor(tmp_path: Path) -> None:
    ...
    with pytest.raises(AuditChainError):
        build_assessment_service(build_container(settings)).assess(
            sample_cases.SIU_CLAIM, actor=sample_cases.ACTOR, tenant=FIXTURE_TENANT
        )

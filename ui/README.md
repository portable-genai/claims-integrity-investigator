...
A Next.js App Router console for Claims Integrity Investigator (`claims-integrity-investigator`).
...
| `app/page.tsx` | The console itself. It reads the service's agent card for its own title, so no product name is hardcoded here. It assesses one claim by id through `POST /v1/assess` (the local profile seeds `CLM-1001` to `CLM-1004`, offered as suggestions) and lists the persona's review queue from `GET /v1/siu-queue`, re-read after every assessment. |

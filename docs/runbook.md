# Runbook: Claims Integrity Investigator (`claims-integrity-investigator`)

## Deploy (gcp)
1. `CLAIMSINTEG_PROFILE=gcp`, install `.[gcp]`, region `asia-southeast1`.
...
3. Ingress is fronted by IAP/Apigee; the app authenticates the S2S caller fail-closed
   (`CLAIMSINTEG_S2S_TOKEN` local, Google OIDC + allowlist secure).
4. **Set `CLAIMSINTEG_IAP_AUDIENCE`.** Without it the service starts, stays
...
`CLAIMSINTEG_IAP_AUDIENCE` is the IAP-protected resource the assertion must be
...
| Symptom | Cause | Fix |
|---|---|---|
| 503, detail names `CLAIMSINTEG_IAP_AUDIENCE` | no audience configured | set it to the protected resource above |
...
| `gcp` IAP assertion | `verified` | authenticated: the signature (against IAP's own key set), the audience (against `CLAIMSINTEG_IAP_AUDIENCE`), the expiry and the issuer are all checked before any claim is read |
...
1. **Nobody chose a profile.** `CLAIMSINTEG_PROFILE` is absent, so no end-user
...
- `add_loopback_exposure_guard` is registered at module scope in `api/app.py`, so it holds under
  `uvicorn claims_integrity_investigator.api.app:app --host 0.0.0.0` (what the Dockerfile `CMD`
...
Set `CLAIMSINTEG_ALLOW_INSECURE_DEMO=1` to accept the exposure deliberately.
...
`CLAIMSINTEG_PROFILE` is read once, in `config.py`, and it has three states:
...
`CLAIMSINTEG_S2S_TOKEN`. With routing on and the URL unset, the managed profile REFUSES TO BOOT,
...
`CLAIMSINTEG_REVIEW_ROUTING` switches routing, read in three states: unset is on, `true`/`false`
...
the failure is logged, and the console says the claim is not queued for review. Terraform
...
`CLAIMSINTEG_AUDIT_ANCHOR` (read by `audit_anchor_path` in
`config/settings.yaml`) to a file on a DIFFERENT volume from
`CLAIMSINTEG_AUDIT_PATH`, ideally writable by a different principal. This is
not decoration:

# ARCHITECTURE: Claims Integrity Investigator (`claims-integrity-investigator`)

Hexagonal ports-and-adapters. A pure-stdlib domain core speaks only to ports (`typing.Protocol`s);
adapter families implement them; one env var (`CLAIMSINTEG_PROFILE`) swaps the
...
## Layout (`src/claims_integrity_investigator/`)

...
def _check_accept(state: dict[str, Any]) -> list[str]:
    facts = _facts(state)
    problems: list[str] = []
    if facts.get("recommendation") != "accept":
        problems.append(
            "the clean claim was not recommended accept: " + str(facts.get("recommendation"))
        )
    if not facts.get("requires_human_review"):
        problems.append("a claim was auto-executed: every disposition is human-approved")
    if not facts.get("review_ref"):
        problems.append("rule R8: an accept disposition was not routed for approval")
    return problems


def _check_siu(state: dict[str, Any]) -> list[str]:
    facts = _facts(state)
    problems: list[str] = []
    if facts.get("recommendation") != "siu_refer":
        problems.append("the organised-fraud claim was not referred to SIU")
    if not facts.get("requires_human_review"):
        problems.append("a high-risk claim did not escalate")
    if not facts.get("review_ref"):

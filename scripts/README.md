...
| `rename_fork.py` | The mechanical half of adopting this repo as a base: rewrites the package name (which is also the console-script name), the `CLAIMSINTEG_` environment prefix, the distribution and resource id, and the Terraform `name_prefix` default, in one pass. It prints a plan and writes nothing without `--yes`. The decisions it cannot make for you are the checklist in `docs/ADOPTING.md`. |
